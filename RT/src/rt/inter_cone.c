...
** Last update Fri Jun  6 07:54:17 2014 Olivier Remy
*/

#include	"funcrt.h"

static	double          calc_c_co(t_eye *eyeb, t_obj *obj)
{
  double        x;
  double        y;
  double        z;
...
static	double          calc_b_co(t_eye *eyeb, t_obj *obj)
{
  double        x;
  double        y;
  double        z;
  ...
  y = eyeb->yeye - obj->eye->yeye;
  z = eyeb->zeye - obj->eye->zeye;
  b = (2 * x * eyeb->Vx) + (2 * y * eyeb->Vy) + (2 * z * eyeb->Vz) * pow(tan(obj->R), 2);
  return (b);
}
