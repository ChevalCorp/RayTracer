...
** Last update Fri Jun  6 17:25:02 2014 marc-aurele lautel
*/
...
void		addcolo(char *str, int i, t_obj *tmp)
{
  ...
  tmp->s_color = malloc(sizeof(t_color));
  tmp->s_color->r = 0;
  tmp->s_color->v = 0;
  tmp->s_color->b = 0;
  pos = 0;
  ...
  tmp->s_color->r = tmp->color->r;
  tmp->s_color->v = tmp->color->v;
  tmp->s_color->b = tmp->color->b;
}
...
void		addtype(char *str, int i, t_obj *tmp)
{
  if (my_strcmp2("plan;", str, i) == 1)
    ...
    tmp->type = 3;
  else if (my_strcmp2("spot;", str, i) == 1)
    tmp->type = 4;
}
